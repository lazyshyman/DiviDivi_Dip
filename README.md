...
```
#### 세부사항
* 3초마다 컴퓨터가 'one', 'rock', 'paper'중 한 가지를 택해서 화면에 보여주고 사용자와 처음 모양이 다르면 
...
## 마치며
아쉬운점 : 변명의 여지 없이 본인의 의지 및 능력부족으로 게임을 완성시키지 못한 것이 매우 아쉽다. 좀 더 공부해서 원하던 게임으로 완성할 수 있어야 할 것 같다...추가로 나중에 다른 dataset을 이용하여 opencv를 활용한 다른 손모양의 인식률을 높여 다른 손모양으로 해볼 수 있으면 좋을 거 같다라는 생각을 했다.
